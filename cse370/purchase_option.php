...
if (isset($_POST['departure']) && isset($_POST['destination']) && isset($_POST['date'])) {
  ...
  $q = "SELECT R.Route_no, R.Date, R.Departure, R.Destination, R.Time, R.Fare, R.Sold_seat, 
  V.Vehicle_type, V.Total_Seat , V.Vehicle_no
  FROM route R, travel_through T, vehicle V WHERE R.Departure = '$departure' and R.Destination = '$destination' 
  ...
  <tbody>
    <?php while ($row = mysqli_fetch_assoc($result)) { ?>
      <tr>
        ...
        <td><?php echo $row["Total_Seat"]-$row["Sold_seat"];?></td>
